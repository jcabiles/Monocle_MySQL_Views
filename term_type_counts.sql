...
use ccgitranscripts;
CREATE OR REPLACE VIEW term_type_counts AS
...
AND 
(
	(MONTH(curdate()) IN (8,9,10,11,12) AND substr(school_year,1,4) = (YEAR(CURDATE())))

	OR

	(MONTH(curdate()) IN (1,2,3,4,5,6,7) AND substr(school_year,1,4) = (YEAR(CURDATE())-1))
)
